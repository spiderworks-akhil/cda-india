      ...
      <div className='container'>
        ...
        <div className='h3'>{HTMLParser(data?.content?.title)}</div>
          ...
          <div className='clients_left'>
            ...
            <Image src={data?.content?.media_id_10?.file_path} alt={data?.content?.media_id_10?.alt_text || 'Our exceptional clients'} width={560} height={530}  />
          </div>
            ...
            <div className='grid grid-cols-3 md:grid-cols-5 gap-[20px] xl:gap-[45px]'>
              ...
              {data?.content?.our_exceptional_clients_listing_id.map((client, index) => (
                <div className='clients_list' key={index}>
                  <Image src={client?.media_id?.file_path} alt={client?.media_id?.alt_text || client?.title || `Client logo ${index + 1}`} width={80} height={35} />
                </div>
              ))}
